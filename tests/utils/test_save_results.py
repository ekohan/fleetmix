...
class TestSaveOptimizationResults(unittest.TestCase):
    ...
    @patch('fleetmix.utils.save_results._write_to_excel')
    @patch('fleetmix.utils.save_results.visualize_clusters')
    def test_save_optimization_results_default_filename(self, mock_visualize, mock_write_excel, mock_datetime):
    ...
    @patch('fleetmix.utils.save_results._write_to_excel')
    @patch('fleetmix.utils.save_results.visualize_clusters')
    def test_save_optimization_results_with_time_measurements(self, mock_visualize, mock_write):
...
class TestVisualizeCluster(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.selected_clusters = pd.DataFrame({
            ...
            'Centroid_Latitude': [4.5, 4.6],
            'Centroid_Longitude': [-74.0, -74.1],
            'Total_Demand': [{'Dry': 100, 'Chilled': 50}, {'Dry': 200, 'Frozen': 100}],
            'Customers': [['A', 'B'], ['C', 'D']],
            'Route_Time': [3.5, 4.2]
        })
