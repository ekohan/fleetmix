...
def setup_cli_assets(tmp_path_factory):
    ...
    mcvrp_file_path = Path(
        f"src/fleetmix/benchmarking/datasets/mcvrp/{MCVRP_INSTANCE}.dat"
    )
    assert mcvrp_file_path.exists(), (
        f"MCVRP benchmark file {mcvrp_file_path} not found."
    )

    cvrp_file_path = Path(
        f"src/fleetmix/benchmarking/datasets/cvrp/{CVRP_INSTANCE}.vrp"
    )
