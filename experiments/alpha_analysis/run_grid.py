...
from experiments.alpha_analysis.config import ALPHA_GRID, C_VALUES, DEMAND_FILES
...
def main():
    ...
    for demand_path, alpha, c, fleet_type in tqdm(combos):
        json_path = RESULTS_RAW / f"{demand_path.stem}_{fleet_type}_{alpha:.2f}_{c:.0f}.json"
        if json_path.exists():
