...
def test_is_customer_feasible_all_goods_fit():
    ...
    customer = pd.Series(
        {"Dry_Demand": 5, "Chilled_Demand": 0, "Frozen_Demand": 0, "Customer_ID": "C1"}
    )
...
def test_is_customer_feasible_missing_compartment():
    ...
    customer = pd.Series(
        {"Dry_Demand": 0, "Chilled_Demand": 3, "Frozen_Demand": 0, "Customer_ID": "C2"}
    )
    config = VehicleConfiguration(
        config_id=2,
        vehicle_type="TestVehicle",
...
def test_is_customer_feasible_capacity_exceeded():
    ...
    customer = pd.Series(
        {"Dry_Demand": 15, "Chilled_Demand": 0, "Frozen_Demand": 0, "Customer_ID": "C3"}
    )
    config = VehicleConfiguration(
        config_id=3,
        vehicle_type="TestVehicle",
