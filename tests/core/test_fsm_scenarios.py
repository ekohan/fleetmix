...
def test_fsm_scenarios(name, clusters, configs, upd, exp):
    ...
    if exp.get("missing_customers"):
        for cust_id in exp["missing_customers"]:
            unique_customers_in_scenario_clusters.add(cust_id)

    if (
        not unique_customers_in_scenario_clusters
        and name == "S12_no_clusters_no_customers"
    ):  # Special case from one of the scenarios
    ...
    for cust_id_for_df in sorted(list(unique_customers_in_scenario_clusters)):
        lat, lon = CUSTOMER_COORDS.get(
            cust_id_for_df, (None, None)
        )  # Fallback for safety
    ...
    if (
        "Customer_ID" not in customers_df.columns and not customer_data_for_df_rebuilt
    ) or not customer_data_for_df_rebuilt:
        customers_df = pd.DataFrame(
            columns=[
                "Customer_ID",
                "Latitude",
