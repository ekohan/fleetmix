...
def setup_cli_assets(tmp_path_factory):
    ...
    cli_inputs_dir = _project_root() / "tests/_assets/cli_inputs"
    ...
    mcvrp_dir = _project_root() / "src/fleetmix/benchmarking/datasets/mcvrp"
    mcvrp_file_path = mcvrp_dir / f"{MCVRP_INSTANCE}.dat"
    
    # Use absolute paths and avoid string interpolation in assert message if possible
    ...
    # Use absolute paths for checking existing directories to be safe
    abs_dirs_to_check = [_project_root() / d for d in dirs_to_check]
    
    pre_existing = {d.name for d in abs_dirs_to_check if d.exists()}

    for d in abs_dirs_to_check:
    ...
    for d in abs_dirs_to_check:
        if d.name in pre_existing:
            continue

        if d.exists() and d.is_dir():
            try:
                base_temp = str(tmp_path_factory.getbasetemp())
                is_tmp_subdir = str(d.resolve()).startswith(base_temp)
            except Exception:
                is_tmp_subdir = False
...
def test_optimize_command_success(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            "optimize",
            "--demand",
            str(MINIMAL_DEMAND_CSV),
            "--config",
            str(MINIMAL_CONFIG_YAML),
            "--output",
            str(specified_output_path),
            "--format",
            "json",
        ],
    )
    ...
    assert result.exit_code == 0, (
        f"Optimize command failed unexpectedly. STDOUT: {result.stdout}\nSTDERR: {result.stderr}"
    )
...
def test_optimize_command_missing_demand(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            "optimize",
            "--demand",
            "non_existent_demand.csv",
            "--config",
            str(MINIMAL_CONFIG_YAML),
            "--output",
            str(
                output_dir
            ),  # Must provide output as it's used before erroring on demand
        ],
    )
    assert result.exit_code != 0, f"STDOUT: {result.stdout}\nSTDERR: {result.stderr}"
    # Typer by default prints exception info to stderr for unhandled ones
    assert (
        "FileNotFoundError" in result.stderr or "Demand file not found" in result.stderr
    )
...
def test_gui_command_mocked(mock_subprocess_run):
    ...
    assert called_args_list[1] == "-m"
    assert called_args_list[2] == "streamlit"
    assert called_args_list[3] == "run"
    assert any("gui.py" in arg for arg in called_args_list)
    assert called_args_list[-2] == "--server.port"  # Ensure it's the second to last arg
    assert called_args_list[-1] == "8888"  # Ensure it's the last arg


INVALID_SYNTAX_CONFIG_YAML = _project_root() / "tests/_assets/configs/invalid_syntax_config.yaml"
...
def test_optimize_command_invalid_config_syntax(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            ...
            str(MINIMAL_DEMAND_CSV),
            "--config",
            str(INVALID_SYNTAX_CONFIG_YAML),
            "--output",
            str(output_dir),
        ],
    )
    assert result.exit_code != 0, (
        f"Command should fail with invalid config. STDOUT: {result.stdout}\nSTDERR: {result.stderr}"
    )
    # Error message might vary depending on YAML parser, check for common terms
    assert (
        "Error loading configuration" in result.stderr
        or "Error parsing YAML" in result.stderr
        or "invalid syntax" in result.stderr.lower()
