...
def test_create_model_counts_traditional_mode(toy_fsm_core_data):
    ...
    assert any('Customer_Coverage_' in name for name in cons_names), f"Expected Customer_Coverage_ constraint, got: {cons_names}"
    assert any('Vehicle_Assignment_' in name for name in cons_names), f"Expected Vehicle_Assignment_ constraint, got: {cons_names}"
...
def test_create_model_counts_split_stop_mode(toy_fsm_core_data):
    ...
    clusters_df.at[0, 'Customers'] = ['C1::dry', 'C1::chilled']  # Two pseudo-customers for same physical customer
    ...
    assert any('Cover_' in name for name in cons_names), f"Expected Cover_ constraint in split-stop mode, got: {cons_names}"
    assert any('Vehicle_Assignment_' in name for name in cons_names), f"Expected Vehicle_Assignment_ constraint, got: {cons_names}"
    # Should have coverage constraints for both goods
    assert any('Cover_C1_dry' in name for name in cons_names), f"Expected Cover_C1_dry constraint, got: {cons_names}"
    assert any('Cover_C1_chilled' in name for name in cons_names), f"Expected Cover_C1_chilled constraint, got: {cons_names}"
...
def test_capacity_violation_model_warning_split_stop_mode(toy_fsm_core_data, caplog):
    ...
    solution = optimization._solve_internal(clusters_df, configurations, customers_df, params)
    
    # Verify that the solution has no selected clusters (empty solution)
    assert len(solution.selected_clusters) == 0, "Should have no selected clusters for infeasible problem"
    assert solution.total_vehicles == 0, "Should have no vehicles used"
    assert solution.missing_customers == set(), "Split-stop mode skips customer validation"
