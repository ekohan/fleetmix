    ...
    if 'vehicles' in params_dict and isinstance(params_dict['vehicles'], dict):
        ...
        for vtype, vdata in params_dict['vehicles'].items():
            if isinstance(vdata, dict) and 'capacity' in vdata and 'fixed_cost' in vdata:
                ...
                # Get per-vehicle operational parameters from session state or use defaults
                avg_speed = st.session_state.get(f'vehicle_{vtype}_avg_speed', 30.0)
                service_time = st.session_state.get(f'vehicle_{vtype}_service_time', 25.0)
                max_route_time = st.session_state.get(f'vehicle_{vtype}_max_route_time', 10.0)
...
def main():
    ...
    with st.sidebar:
        ...
        # Operations parameters - only variable cost per hour remains global
