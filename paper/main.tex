...
Throughout this section, we employ the same parameters and computational environment described in Section \ref{sec:synthetic_results}. For transparency and reproducibly, our instances and \red{detailed computational results (including run times, which are under X minutes across all instances and parameter combinations tested)} are publicly available at \url{https://github.com/ekohan/fleetmix/tree/main/src/fleetmix/benchmarking/datasets/case}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Generic Paper/Imagens/service_area.pdf}
...
Figure \ref{fig:mean_fleet_and_cost_reduction} illustrates how the average fleet size and total cost change when varying a single parameter by –50\%, –20\%, +20\%, and +50\% relative to the baseline scenario. Results consistently show that adopting \acp{MCV} leads to reductions in both metrics across all parameter combinations. Notably, in the baseline scenario (indicated by the vertical line at 0\% on the horizontal axis of each subplot), using \acp{MCV} reduces the average fleet size by 7.5 vehicles and the average total cost by 14.7\%. To facilitate the interpretation of these results, Table \ref{tab:sensitivity_analysis} reports additional operational metrics: vehicle load utilization (i.e., the average percentage of total vehicle capacity used), the average number of customers served per vehicle, the average route duration (including travel and service times), and the fleet composition (in terms of vehicle types used). We now examine the main direct effects of each parameter variation.
...
\midrule
        \multirow{5}[2]{*}{Load Capacity } & -50\% &       & 87.0\% & 85.9\% &       & 10.0  & 12.2  &       & 5.3   & 6.6   &       & 6.5   & 3.3   & 16.7 \\
          & -20\% &       & 83.6\% & 79.6\% &       & 11.7  & 13.3  &       & 6.1   & 7.1   &       & 8.1   & 4.0   & 10.1 \\
          & 0\%   &       & 81.4\% & 76.0\% &       & 12.8  & 13.7  &       & 6.6   & 7.3   &       & 8.2   & 4.5   & 7.4 \\
          & 20\%  &       & 79.7\% & 71.8\% &       & 13.3  & 14.1  &       & 6.9   & 7.5   &       & 9.8   & 4.2   & 5.3 \\
          & 50\%  &       & 76.1\% & 66.5\% &       & 13.8  & 14.4  &       & 7.1   & 7.7   &       & 11.3  & 3.7   & 3.5 \\
    \midrule
    \multirow{5}[2]{*}{Service Time} & -50\% &       & 83.6\% & 78.4\% &       & 16.9  & 19.1  &       & 5.0   & 5.9   &       & 4.1   & 2.7   & 9.2 \\
          & -20\% &       & 82.6\% & 76.9\% &       & 14.2  & 15.4  &       & 6.1   & 6.8   &       & 6.2   & 4.0   & 8.1 \\
          & 0\%   &       & 81.4\% & 76.0\% &       & 12.8  & 13.7  &       & 6.6   & 7.3   &       & 8.2   & 4.5   & 7.4 \\
          & 20\%  &       & 80.4\% & 73.9\% &       & 11.4  & 12.4  &       & 6.9   & 7.7   &       & 11.5  & 4.4   & 6.6 \\
          & 50\%  &       & 78.5\% & 70.3\% &       & 10.0  & 10.9  &       & 7.3   & 8.1   &       & 15.9  & 4.2   & 5.8 \\
    \midrule
    \multirow{5}[2]{*}{Max. Route Duration } & -50\% &       & 70.6\% & 57.1\% &       & 7.4   & 8.0   &       & 4.0   & 4.4   &       & 27.6  & 2.9   & 4.2 \\
          & -20\% &       & 80.4\% & 72.3\% &       & 10.9  & 11.8  &       & 5.7   & 6.3   &       & 13.1  & 4.7   & 6.1 \\
          & 0\%   &       & 81.4\% & 76.0\% &       & 12.8  & 13.7  &       & 6.6   & 7.3   &       & 8.2   & 4.5   & 7.4 \\
          & 20\%  &       & 82.5\% & 78.2\% &       & 14.1  & 15.3  &       & 7.3   & 8.1   &       & 6.3   & 4.1   & 8.1 \\
          & 50\%  &       & 82.3\% & 78.9\% &       & 15.7  & 17.6  &       & 8.0   & 9.3   &       & 4.7   & 2.7   & 9.2 \\
    \bottomrule
    \end{tabular}%
    }
...
Figure \ref{fig:mean_fleet_and_cost_reduction} shows that vehicle capacity has only a modest effect on fleet size for both \ac{MCV} and \ac{SCV} cases. For instance, a 50\% reduction in the total vehicle capacity results in only a slight increase in the number of vehicles (approximately from 20.2 to 26.5 for \acp{MCV} and from 27.7 to 31 for \acp{SCV}) yet leads to a sharp rise in total cost. 
Table \ref{tab:sensitivity_analysis} supports these findings: when load capacity is reduced, the fleet composition shifts toward a much higher share of large, more expensive Type C vehicles (from 7.4 to 16.7 vehicles on average), while the number of small, less expensive Type A vehicles decreases. 
...
As shown in Figure \ref{fig:mean_fleet_and_cost_reduction}, the response to changes in maximum route duration is highly asymmetric. For instance, when the maximum route duration is reduced by 50\%, the number of vehicles increases by approximately 70\% in both \ac{MCV} and \ac{SCV} cases, resulting in a substantial increase in total cost: 23\% for \ac{MCV} and 27\% for \acp{SCV}. Conversely, extending the maximum route duration by 50\% (corresponding to 15 working hours, which exceeds practical operational limits but is included here for illustrative purposes) yields only moderate efficiency gains: the average fleet size decreases by 18\% and 22\% for the \ac{MCV} and \ac{SCV} cases, respectively, with total cost reductions of only 6.5\% and 5.9\%.
This asymmetric response is mainly driven by fleet composition: with a 50\% reduction in maximum route duration, small vehicles (Type A) rise quickly from 8.2 to 27.6, since limited time on route prevents efficient use of (more expensive) larger vehicles. In contrast, when duration is extended by 50\%, the share of large vehicles (Type C) increases from 7.4 to 9.2, but their higher capacity does not translate into a substantial reduction in total fleet size compared to the baseline scenario (e.g., when using \acp{MCV}, the total fleet size is reduced from 20.1 to 16.6 vehicles).
...
\paragraph{Effects of service time} Figure \ref{fig:mean_fleet_and_cost_reduction} shows that customer service time exhibits the most linear relationship among the parameters studied. If the company manages to reduce service time by 50\% (e.g., through better coordination with customers regarding product unloading and delivery), the average number of \acp{MCV} can be reduced by 4.3 vehicles, while achieving a reduction of 7.8 vehicles when employing a traditional \ac{SCV} fleet.
This pattern is also reflected in Table \ref{tab:sensitivity_analysis}. As service time decreases, vehicles can serve more customers per vehicle in a nearly proportional way (from 12.8 to 16.9 customers for \acp{MCV} and from 13.7 to 19.1 for \acp{SCV} under a 50\% reduction). Similarly, the effective route duration follows the same trend, decreasing almost linearly as service time is reduced (from 6.6 to 5.0 hours for \acp{MCV} and from 7.3 to 5.9 for \acp{SCV}). Because both customers-per-vehicle and route duration scale almost directly with the service time parameter, the total number of required vehicles decreases at a nearly constant rate as service time reduces.
...
For this analysis, we establish a baseline fixed cost of $100$ units for using one \ac{SCV}. The fixed cost for using one \ac{MCV} with configuration $v \in V$ is set as $100 \cdot \left( \alpha + c \cdot (|M_v| - 1) \right)$, where $\alpha \in [1, 2]$ represents the premium factor associated with using \acp{MCV}; $M_v$, with $\emptyset \neq M_v \subseteq M$, is the subset of compartments of vehicle configuration $v \in V$; and $c \in [0, 0.5]$ denotes a fraction of the \ac{SCV} fixed costs incurred per additional compartment. For example, when $\alpha = 1.5$ and $c = 0.2$, the total fixed cost of using one \ac{MCV} with three compartments is $190 = 100 \cdot (1.5 + 0.2 \cdot (3 - 1))$, as there are two additional compartments. All other operational parameter values (e.g., customer service time, variable costs) remain the same as defined in Section \ref{sec:real_world_instances}.
 ...
 \begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Generic Paper/Imagens/heatmap_mcv_adoption_landscape.pdf}
    \vspace{0.4mm}
...
Figure \ref{fig:heatmap} shows a clear nonlinear decline in \ac{MCV} adoption as both cost factors increase. When \acp{MCV} are relatively inexpensive compared to \acp{SCV} (e.g., when $\alpha \leq 1.4$ and $c \leq 0.1$, or $\alpha \leq 1.2$ and $c \leq 0.2$), their share consistently represents 90\% or more, indicating that \acp{MCV} dominate the fleet composition and achieve full adoption across all days. However, even modest increases in either parameter rapidly reduce their utilization. Beyond $\alpha = 1.5$ and $c \geq 0.3$, the average share of \acp{MCV} falls below 50\%, suggesting that their fixed cost quickly outweighs the operational flexibility offered by compartmentalization. Overall, Figure \ref{fig:heatmap} highlights that \ac{MCV} adoption is highly sensitive to the underlying cost structure, indicating that only specific cost parameters make \acp{MCV} an operationally dominant option for fleet composition.
...
The results of our real-world case study provide valuable insights for food distributors and fleet managers considering adopting \acp{MCV}. Below, we summarize the key managerial implications of our analysis.

% Structure:
\blue{\paragraph{Summarize the insight} Explain the insight based on the results. Provide practical implications.}

%%%% COMMENTS SO FAR: %%%%%

% Mention that the outcomes in this chapter come from the demand data that was used, in which customers make frequent purchase orders with smaller volume and therefore require frequent visits to deliver goods.

% Vehicles are constrained by physical load capacity and maximum route duration. Our results show that there is a modest effect in reducing vehicle size (which can help cities as there is a move towards smaller vehicles) but a strong effect in reducing the maximum time vehicles are allowed to operate.
    
% \acfp{MCV} with flexible compartment capacity achieves higher average load utilization than \acfp{SCV}, as they can carry incompatible products requested by different customers, enabling better product allocation per vehicle.
    
% \acp{MCV} routes are generally shorter than \acp{SCV} routes because \acp{MCV} can satisfy multi-product demands, serving fewer customers per route while maximizing vehicle load. \acp{SCV}, restricted to one product type, requires more stops and longer route times to justify their deployment.
    
% Mixed fleets combining \acp{MCV} and \acp{SCV} operate with fewer vehicles compared to heterogeneous fleets composed only of \acp{SCV} fleets, since \acp{MCV} can consolidate multiple product types in a single trip. As fewer vehicles are needed, routes are shorter, and load utilization is improved, total costs in mixed fleets are consistently lower than \acp{SCV} fleets.
    
% Reducing customer service time is the most effective strategy to improve service levels and vehicle utilization, while simultaneously decreasing the required fleet size and total operational costs for both \acp{SCV} and \acp{MCV} fleets.

% Incentives for customers to consolidade purchases would also be beneficial along the month, reducing the number of visits to a single customer. %Increasing maximum route duration, also changes the time variable, but does not bring the same level of benefits as reducing the stop times, thefore this should not be the main driver to bring benefits.

% Compartment setup cost is a key variable when defining the fleet. The experiment shows that the compartment setup cost is very important when defining the ideal fleet. The real costs should be as precise as possible to guarantee the optimality when deciding the fleet.

% If there is a reasonable demand fluctuation is present, as in our dataset, the number of vehicles on each day can change substantially from peak to lower demand days. Some strategies must be in place to accomodate for this variation withou incurring in high costs of stopped vehicles on low demand days or unserved customers on high demand days. Some strategies may be sharing fleet with other business or having contractors available to cover for higher demand days.
