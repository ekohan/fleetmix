...
from experiments.alpha_analysis.metrics import (
    cost_per_drop,
    cost_per_kg,
    split_rate,
...
def run_day(demand_path: Path, fleet_params: FleetmixParams, fleet_type: str, alpha: float = 1.0, C: float = 0.0) -> dict:
    ...
    total_demand_kg = customers_df[[col for col in customers_df.columns if col.endswith('_Demand')]].sum().sum()
    ...
    base_fixed_cost = float(next(iter(base_params.problem.vehicles.values())).fixed_cost)
    ...
    total_route_time_hours = solution.total_variable_cost / fleet_params.problem.variable_cost_per_hour if fleet_params.problem.variable_cost_per_hour > 0 else 0.0
