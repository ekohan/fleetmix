...
def find_paradoxical_blocks(threshold_mcv_share: float = 0.4) -> List[Tuple[float, float]]:
    """
    ...
    for json_file in RESULTS_RAW.glob("*_MIXED_*.json"):
        ...
        try:
            ...
            more_visits = mixed_data.get("average_visits_per_customer", 0) > scv_data.get("average_visits_per_customer", 0)
            lower_cost = mixed_data.get("total_cost", float('inf')) < scv_data.get("total_cost", float('inf'))
            low_mcv_share = 0 < mixed_data.get("mcv_share", 0) <= threshold_mcv_share
            
            if (more_visits and lower_cost) or (low_mcv_share and mixed_data.get("mcv_vehicles", 0) > 0):
...
def run_specific_blocks(
    ...
    if demand_days:
        demand_files = [d for d in DEMAND_FILES if any(day in d.stem for day in demand_days)]
    else:
    ...
    print(f"C values: {cs}")
    print(f"Blocks to run: {len(alphas)} × {len(cs)} = {len(alphas) * len(cs)} (alpha, C) combinations")
    ...
    if len(demand_files) == 70:
        print(f"Estimated runtime: {len(alphas) * len(cs) * 30}-{len(alphas) * len(cs) * 45} minutes")
    print()
            ...
            for alpha in alphas:
                for C in cs:
                    ...
                    if json_path.exists() and not force_rerun:
                        with open(json_path, "r") as f:
                            data = json.load(f)
                        pbar.set_description(f"Loading {demand_path.stem} α={alpha} C={C}")
                    else:
                        pbar.set_description(f"Running {demand_path.stem} α={alpha} C={C}")
    ...
    baselines = df[df["fleet_type"] == "SCV_BASE"][["instance", "total_cost", "average_visits_per_customer"]].rename(
        columns={"total_cost": "scv_cost", "average_visits_per_customer": "scv_avg_visits"}
    )
    mixed = df[df["fleet_type"] == "MIXED"].merge(baselines, on="instance", how="left")
    mixed["delta_cost_pct"] = 100.0 * (mixed["total_cost"] - mixed["scv_cost"]) / mixed["scv_cost"]
    mixed["delta_visits_pct"] = 100.0 * (mixed["average_visits_per_customer"] - mixed["scv_avg_visits"]) / mixed["scv_avg_visits"]
...
def main():
    parser = argparse.ArgumentParser(description="Run specific (alpha, C) blocks for targeted analysis")
    parser.add_argument("--alpha", type=str, help="Alpha values (comma-separated, e.g., '1.0,1.1')")
    parser.add_argument("--c", type=str, help="C values (comma-separated, e.g., '30,50')")
    parser.add_argument("--days", type=str, help="Specific demand days (comma-separated)")
    parser.add_argument("--paradox", action="store_true", help="Auto-detect and run paradoxical blocks")
    ...
    if not mixed_only.empty:
        ...
        if not paradox_cases.empty:
            ...
            if 'avg_vehicle_utilization_pct' in paradox_cases.columns:
                print(f"  Avg utilization in paradox cases: {paradox_cases['avg_vehicle_utilization_pct'].mean():.1f}%")
                print(f"  Avg MCV utilization: {paradox_cases['avg_mcv_utilization_pct'].mean():.1f}%")
                print(f"  Avg SCV utilization: {paradox_cases['avg_scv_utilization_pct'].mean():.1f}%")
            else:
                print("  (Utilization data not available - rerun with --force to generate)")
