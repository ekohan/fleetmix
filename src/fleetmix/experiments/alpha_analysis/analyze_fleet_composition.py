...
def compute_multi_good_proportion(instance_name: str) -> float:
    ...
    try:
        ...
        if 'ClientID' in customers_df.columns and 'ProductType' in customers_df.columns:
            ...
            products_per_customer = customers_df.groupby('ClientID')['ProductType'].nunique()
            ...
            return multi_good_customers / total_customers if total_customers > 0 else 0.0
        else:
...
def load_mixed_results():
    ...
    print(f"  - Multi-good % range: {mixed['multi_good_pct'].min():.1%} - {mixed['multi_good_pct'].max():.1%}")
...
def compute_composition_metrics(df):
    ...
    metrics = df.groupby(["alpha", "C"]).agg({
        "mcv_share": ["mean", "median", "std", "min", "max"],
        "mcv_vehicles": [
            ("adoption_days", lambda x: (x > 0).sum()),  # Days with at least 1 MCV
            ("pure_mcv_days", lambda x: (df.loc[x.index, 'mcv_share'] >= 0.99).sum()),
            ("mean", "mean"),
        ],
        "scv_vehicles": "mean",
    ...
    metrics['adoption_probability'] = metrics['mcv_vehicles_adoption_days'] / metrics['instance_count']
    
    # Compute regime distribution
    regime_dist = df.groupby(["alpha", "C", "regime"]).size().unstack(fill_value=0)
    regime_dist = regime_dist.div(regime_dist.sum(axis=1), axis=0) * 100  # Convert to percentages
    ...
    print(f"  Range: {metrics['mcv_share_mean'].min():.1%} - {metrics['mcv_share_mean'].max():.1%}")
    print(f"\nAdoption days:")
    print(f"  Mean: {metrics['mcv_vehicles_adoption_days'].mean():.1f}/70")
    print(f"  Range: {metrics['mcv_vehicles_adoption_days'].min()}-{metrics['mcv_vehicles_adoption_days'].max()}/70")
...
def chart1_mcv_adoption_landscape(metrics):
    ...
    pivot_adoption = metrics.pivot(index="alpha", columns="C", values="mcv_vehicles_adoption_days")
    pivot_pure_mcv = metrics.pivot(index="alpha", columns="C", values="mcv_vehicles_pure_mcv_days")
    
    # Create heatmap background (MCV share)
    im = ax.imshow(pivot_share.values * 100, cmap="YlGnBu", aspect="auto", 
    ...
    ax.set_yticklabels([f"{a:.1f}" if a != int(a) else f"{int(a)}.0" 
                        for a in pivot_share.index])
    
    ax.set_xlabel("C: Compartment Setup Cost (% of SCV fixed cost)", fontsize=12, weight='bold')
    ax.set_ylabel("α: MCV Fixed Cost Multiplier", fontsize=12, weight='bold')
    ax.set_title("MCV Adoption Landscape\nValues: MCV Share% | Adoption Days/70 | (Pure MCV Days★)", 
...
def chart2_fleet_composition_regimes(regime_dist, metrics):
    ...
    selected_params = [
        (1.0, 0), (1.0, 20), (1.1, 0), (1.1, 30),
        (1.2, 10), (1.2, 40), (1.3, 20), (1.3, 50),
        (1.4, 30), (1.5, 40), (1.6, 50), (1.8, 50), (2.0, 50)
    ]
    ...
    for regime in regimes:
        if regime in regime_subset.columns:
            ...
            ax.bar(labels, values, bottom=bottom, label=regime, 
                  color=regime_colors[regime], edgecolor='white', linewidth=1.5)
    ...
    ax.set_xlabel("Parameter Configuration (α: MCV Cost Multiplier, C: Compartment Cost)", 
                 fontsize=12, weight='bold')
    ax.set_title("Fleet Composition Regime Classification Across Parameter Space\n" +
...
def chart3_mcv_adoption_probability_surface(metrics):
    ...
    # Interpolate
    prob_mesh = griddata((alpha_vals, c_vals), adoption_prob, 
                         (alpha_mesh, c_mesh), method='cubic')
    
    # Create filled contour plot
    levels = np.linspace(0, 1, 21)
    contourf = ax.contourf(alpha_mesh, c_mesh, prob_mesh, levels=levels, 
                          cmap='RdYlGn', alpha=0.8)
    
    # Add contour lines at key adoption thresholds
    contour_lines = ax.contour(alpha_mesh, c_mesh, prob_mesh, 
    ...
    contour_50 = ax.contour(alpha_mesh, c_mesh, prob_mesh, 
                            levels=[0.5], colors='red', linewidths=3)
    
    # Scatter actual data points
    ax.scatter(alpha_vals, c_vals, c='black', s=50, marker='o', 
              edgecolors='white', linewidths=1, zorder=5, alpha=0.7)
    ...
    ax.set_title("MCV Adoption Probability Surface\n" +
...
def chart4_fleet_composition_variability(df_full):
    ...
    for idx, (alpha, C) in enumerate(selected_params):
        ...
        # Plot
        ax.scatter(subset['day_index'], subset['mcv_share'] * 100, 
                  c=colors, s=30, alpha=0.7, edgecolors='black', linewidths=0.5)
        
        # Add mean line
        mean_share = subset['mcv_share'].mean() * 100
        ax.axhline(mean_share, color='blue', linestyle='--', linewidth=2, 
    ...
    fig.suptitle("Fleet Composition Variability Across Demand Days\n" +
    ...
    legend_elements = [
        ...
        Patch(facecolor='#7dcea0', edgecolor='black', label='MCV-dominant (50-99% MCV)'),
...
def chart5_fleet_size_reduction_vs_mcv_adoption(df_full, scv_baseline):
    ...
    df_analysis['fleet_reduction_pct'] = (
        ...
        df_analysis['scv_baseline_vehicles'] * 100
    )
    ...
    for regime in ["Low cost", "Medium cost", "High cost"]:
        ...
        ax_main.scatter(subset['mcv_share'] * 100, subset['fleet_reduction_pct'],
                       c=regime_colors[regime], label=regime, alpha=0.5, s=20)
    ...
    if len(x_clean) > 10:
        ...
        ax_main.text(0.05, 0.95, f"Correlation: r={corr:.3f}, p<0.001" if p_value < 0.001 else f"Correlation: r={corr:.3f}, p={p_value:.3f}",
                    transform=ax_main.transAxes, fontsize=11, weight='bold',
                    verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    ax_main.set_xlabel("MCV Share (%)", fontsize=12, weight='bold')
    ax_main.set_ylabel("Fleet Size Reduction vs SCV Baseline (%)", fontsize=12, weight='bold')
    ...
    ax_top.hist(df_analysis['mcv_share'] * 100, bins=50, color='skyblue', 
    ...
    ax_right.hist(df_analysis['fleet_reduction_pct'], bins=50, 
                 orientation='horizontal', color='lightcoral', 
                 edgecolor='black', alpha=0.7)
    ax_right.set_xlabel("Frequency", fontsize=10)
    ax_right.tick_params(labelleft=False)
    
    fig.suptitle("Fleet Size Reduction vs MCV Adoption\n" +
...
def chart6b_multigood_by_cost_level(df_full):
    ...
    # Plot bars
    bars1 = ax.bar(x - width/2, results_df['low_mean'], width, 
                   label=f'Low Multi-Good (<{median_mg:.1%})',
                   color='#3498db', edgecolor='black', linewidth=1.5,
                   yerr=results_df['low_se'], capsize=5, error_kw={'linewidth': 2})
    
    bars2 = ax.bar(x + width/2, results_df['high_mean'], width,
                   label=f'High Multi-Good (≥{median_mg:.1%})',
                   color='#2ecc71', edgecolor='black', linewidth=1.5,
                   yerr=results_df['high_se'], capsize=5, error_kw={'linewidth': 2})
...
def chart6c_multigood_tercile_bars(df_full):
    ...
    # Plot bars
    bars = ax.bar(range(3), stats_df['mean_mcv'], 
                  color=colors, edgecolor='black', linewidth=2,
                  yerr=stats_df['se'], capsize=8, error_kw={'linewidth': 2})
    ...
    for i, (bar, row) in enumerate(zip(bars, stats_df.itertuples())):
        ...
        ax.text(i, -0.15, f'{row.mg_range}\n(N={row.n})',
               ha='center', va='top', fontsize=10)
    
    # Add arrows and deltas
    # Low to Medium
    delta1 = stats_df.iloc[1]['mean_mcv'] - stats_df.iloc[0]['mean_mcv']
    ax.annotate('', xy=(1, stats_df.iloc[1]['mean_mcv']), 
                xytext=(0, stats_df.iloc[0]['mean_mcv']),
                arrowprops=dict(arrowstyle='->', color='black', lw=2))
    ax.text(0.5, (stats_df.iloc[0]['mean_mcv'] + stats_df.iloc[1]['mean_mcv'])/2 + 0.05,
           f'+{delta1:.1%}', ha='center', fontsize=11, weight='bold')
    
    # Medium to High
    delta2 = stats_df.iloc[2]['mean_mcv'] - stats_df.iloc[1]['mean_mcv']
    ax.annotate('', xy=(2, stats_df.iloc[2]['mean_mcv']), 
                xytext=(1, stats_df.iloc[1]['mean_mcv']),
                arrowprops=dict(arrowstyle='->', color='black', lw=2))
    ax.text(1.5, (stats_df.iloc[1]['mean_mcv'] + stats_df.iloc[2]['mean_mcv'])/2 + 0.05,
           f'+{delta2:.1%}', ha='center', fontsize=11, weight='bold')
    ...
    ax.set_ylabel('Mean MCV Share', fontsize=13, weight='bold')
    ax.set_title('MCV Adoption Increases with Multi-Good Customer Proportion\nSweet Spot: 1.2≤α<2.0, 20%≤C<50%',
                fontsize=14, weight='bold', pad=20)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{y*100:.0f}%'))
